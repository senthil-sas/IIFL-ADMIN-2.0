...
const { androidCount, iosCount, webCount, apiCount, loading: detailsLoading } = useLoginDetails()
...
interface ChartMinute {
  minute: string
  apiLoginCount: number
  androidLoginCount: number
  iosLoginCount: number
  webLoginCount: number
  totalCount: number
}

const chartLoading = ref(true)
const minuteData = ref<ChartMinute[]>([])

onMounted(async () => {
  chartLoading.value = true
  try {
    const res = await httpService.getLoginChart()
    if (res.data?.result) minuteData.value = res.data.result
  } finally {
...
function toIdx(dt: string): number {
  const time = (dt.split(' ')[1] ?? '00:00:00')
  const [h, m] = time.split(':').map(Number)
  return h * 60 + m
}

const minuteLogins = computed(() => {
  const arr = new Array<number>(SESSION).fill(0)
  minuteData.value.forEach((d) => { arr[toIdx(d.minute)] = d.totalCount })
  return arr
})

const deviceMap = computed(() => {
  const m = new Map<number, ChartMinute>()
  minuteData.value.forEach((d) => m.set(toIdx(d.minute), d))
  return m
})

const nowIdx = computed(() => {
  if (!minuteData.value.length) return 0
  return toIdx(minuteData.value[minuteData.value.length - 1].minute)
})

const cumLogins = computed(() =>
  minuteLogins.value.reduce<number[]>((acc, v, i) => {
    acc.push((acc[i - 1] ?? 0) + v)
    return acc
  }, [])
)

const totalToday = computed(() => cumLogins.value[nowIdx.value] ?? 0)

const avgCum = computed(() =>
  Array.from({ length: SESSION }, (_, i) => {
    if (!totalToday.value) return 0
    const t = i / SESSION
    return Math.round((totalToday.value * 0.94) / Math.tanh(3) * Math.tanh(t * 3))
  })
)

// ── Donut — sourced from /adminrest/user/login/details ────────────────────────
const dist = computed(() => [
...
const donutSegs = computed(() => {
  ...
  return dist.value.map((d) => {
    const len = donutSum.value ? (d.n / donutSum.value) * C : 0
...
const lineMax = computed(() =>
  Math.max(cumLogins.value[nowIdx.value] ?? 0, avgCum.value[SESSION - 1]) * 1.06 || 1
)
const ys = (v: number) => pad.t + innerHL - (v / lineMax.value) * innerHL

const todayLine = computed(() =>
  cumLogins.value.slice(0, nowIdx.value + 1)
    .map((v, i) => `${i ? 'L' : 'M'}${xs(i)},${ys(v)}`).join(' ')
)
const avgLine = computed(() =>
  avgCum.value.map((v, i) => `${i ? 'L' : 'M'}${xs(i)},${ys(v)}`).join(' ')
)
const todayArea = computed(() =>
  `${todayLine.value} L${xs(nowIdx.value)},${pad.t + innerHL} L${xs(0)},${pad.t + innerHL} Z`
)
const nowX = computed(() => xs(nowIdx.value))
const nowY = computed(() => ys(cumLogins.value[nowIdx.value] ?? 0))

const barMaxV = computed(() =>
  Math.max(...minuteLogins.value.slice(0, nowIdx.value + 1)) || 1
)
const bars = computed(() =>
  minuteLogins.value
    .map((v, i) => ({ v, i }))
    .filter((b) => b.v > 0 && b.i <= nowIdx.value)
    .map((b) => {
      const h = (b.v / barMaxV.value) * (HBar - 18)
...
const onMove = (e: MouseEvent) => {
  ...
  hover.value = { idx: Math.max(0, Math.min(SESSION - 1, Math.round(t * (SESSION - 1)))) }
}
const onLeave = () => (hover.value = null)

const tip = computed(() => {
  if (!hover.value || chartLoading.value) return null
  const i = hover.value.idx
  const tx = xs(i)
  const inToday = i <= nowIdx.value
  const ty = inToday ? ys(cumLogins.value[i]) : ys(avgCum.value[i])
  ...
  const loginsMin = inToday ? (minuteLogins.value[i] ?? 0) : null
  const d = deviceMap.value.get(i)
  const rows = d
    ? [
        { lbl: 'Android', n: d.androidLoginCount, c: '#1B5BD9' },
        { lbl: 'iOS',     n: d.iosLoginCount,     c: '#7C3AED' },
        { lbl: 'Web',     n: d.webLoginCount,      c: '#5B8DEF' },
        { lbl: 'APIs',    n: d.apiLoginCount,      c: '#A6B6D4' },
      ]
    : []
  return {
    i, tx, ty, inToday, flipLeft, label, loginsMin, rows,
    totalLoginsToday: cumLogins.value[nowIdx.value],
  }
})
const tipPct = (n: number, total: number) => (total ? Math.round((n / total) * 100) : 0)
</script>
  ...
  <div class="card">
    <div class="card-head">
      ...
      <span class="badge" style="background:rgba(15,157,110,0.10);color:var(--emerald);border-color:rgba(15,157,110,0.25)">
        <span class="badge-dot" style="background:var(--emerald)"></span>Login API · live
      </span>
    ...
    <div class="card-body" style="padding-bottom:12px">
      ...
      <div style="display:flex;gap:56px;margin-bottom:14px">
        ...
        <div>
          ...
          <div v-if="chartLoading" class="skel-line" style="height:38px;width:100px;margin-top:4px" />
          <div v-else :style="{ fontFamily:'var(--display)', fontSize:'34px', fontWeight:700, letterSpacing:'-0.02em', color:accent, lineHeight:1.1, marginTop:'2px' }">
        ...
        <div>
          ...
          <div v-if="chartLoading" class="skel-line" style="height:38px;width:100px;margin-top:4px" />
          <div v-else style="font-family:var(--display);font-size:34px;font-weight:700;letter-spacing:-0.02em;color:var(--muted);line-height:1.1;margin-top:2px">
            {{ fmt(avgCum[nowIdx]) }}<span style="font-size:14px;font-weight:500;opacity:0.85;margin-left:6px">logins</span>
          </div>
        </div>
        <!-- Donut -->
        <div style="margin-left:auto;align-self:center;display:flex;align-items:center;gap:12px">
          <div v-if="detailsLoading" class="skel-line" style="width:60px;height:60px;border-radius:50%" />
          <svg v-else viewBox="0 0 60 60" width="60" height="60" style="transform:rotate(-90deg);flex-shrink:0">
          ...
          <div style="display:flex;flex-direction:column;gap:2px;font-size:12px;font-family:var(--sans)">
            <template v-if="detailsLoading">
              <div v-for="n in 4" :key="n" class="skel-line" style="height:14px;width:90px;margin-bottom:2px" />
            </template>
            <template v-else>
              <div v-for="d in dist" :key="d.lbl" style="display:flex;align-items:center;gap:6px;color:var(--ink)">
                ...
                <span style="font-family:var(--mono);color:var(--muted)">{{ donutSum ? Math.round((d.n / donutSum) * 100) : 0 }}%</span>
              </div>
            </template>
          </div>
        </div>
      ...
      <div style="position:relative">
        <!-- Skeleton overlay -->
        <div v-if="chartLoading" class="chart-skel">
          <div class="skel-line" style="width:100%;height:100%;border-radius:6px" />
        </div>
        ...
        >
          ...
          <text
            v-for="(idx, k) in tickIdx" :key="'tl' + k"
            :x="xs(idx)" :y="H - 6"
            :text-anchor="k === 0 ? 'start' : k === tickIdx.length - 1 ? 'end' : 'middle'"
            font-size="11" font-family="var(--mono)" fill="rgba(15,18,28,0.55)"
          >{{ tickLabels[k] }}</text>
          <line v-for="(idx, k) in tickIdx" :key="'tt' + k" :x1="xs(idx)" :x2="xs(idx)" :y1="barBot" :y2="barBot + 4" stroke="rgba(15,18,28,0.3)" stroke-width="1" />
        ...
        >
          ...
          <div style="display:flex;justify-content:space-between;gap:18px;margin-bottom:4px">
            ...
            <span :style="{ fontFamily:'var(--display)', fontSize:'16px', fontWeight:700, color:accent }">{{ tip.totalLoginsToday?.toLocaleString() ?? '—' }}</span>
          </div>
          ...
          <template v-if="tip.rows.length">
            ...
            <div v-for="row in tip.rows" :key="row.lbl" style="display:grid;grid-template-columns:60px 1fr 36px;align-items:center;gap:8px;margin-bottom:4px">
              ...
              <span style="position:relative;height:4px;background:rgba(255,255,255,0.08);border-radius:2px;overflow:hidden">
                <span :style="{ position:'absolute', inset:0, width:`${tipPct(row.n, tip.loginsMin ?? 0)}%`, background:row.c, borderRadius:'2px' }" />
              </span>
